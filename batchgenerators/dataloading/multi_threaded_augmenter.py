...
def producer(queue, data_loader, transform, thread_id, seed, abort_event, purpose):
    ...
    item = None
    if (purpose == "val"):
        logging.debug(str(datetime.now().time()) + " " + purpose +" "+ str(thread_id) + ' : Producer initiated ')
    while True:
        ...
        if not abort_event.is_set():
            ...
            if item is None:
                try:
                    if(purpose == "val"):
                        logging.debug(str(datetime.now().time()) + " " + purpose +" "+ str(thread_id) + ' : Getting item ')
                    item = next(data_loader) 
                    if(purpose == "val"): 
                        logging.debug(str(datetime.now().time()) + " " + purpose +" "+ str(thread_id) + ' : Got item ')            
                    if transform is not None:
                        item = transform(**item)
                    if(purpose == "val"):
                        logging.debug(str(datetime.now().time()) + " " + purpose +" "+ str(thread_id) + ' : Transformed ') 
                except StopIteration:
                    item = "end"

            try:
                if(purpose == "val"):
                    logging.debug(str(datetime.now().time()) + " " + purpose +" "+ str(thread_id) + ' : Push Start. Queue Length : ' + str(queue.qsize()))
                queue.put(item, timeout=2)
                if(purpose == "val"):
                    logging.debug(str(datetime.now().time()) + " " + purpose +" "+ str(thread_id) + ' : Push Complete. Queue Length : ' + str(queue.qsize()))
                item = None
            except Full:
...
class MultiThreadedAugmenter(object):
            ...
            try:
                if self.abort_event.is_set():
                    ...
                    if not self.pin_memory:
                        ...
                        if(self.purpose == "val"):
                            logging.debug(str(datetime.now().time()) + " " + self.purpose + " " + str(use_this_queue) + ' popped. Queue Length : ' + str(self._queues[use_this_queue].qsize()))
                    else:
    ...
    def _start(self):
        if len(self._processes) == 0:
            ...
            logging.debug("Starting workers at : " + str(datetime.now().time()))
            self._queue_loop = 0
            self._end_ctr = 0
            ...
            with threadpool_limits(limits=1, user_api="blas"):
                for i in range(self.num_processes):
                    ...
                    self._processes.append(Process(target=producer, args=(self._queues[i], self.generator, self.transform, i, self.seeds[i], self.abort_event, self.purpose)))
                    self._processes[-1].daemon = True
                    self._processes[-1].start()
                    logging.debug(str(datetime.now().time()) + " " + self.purpose +" "+ str(i) + ' : Spawned. ')
